...
- **Проверка ключей**: ПРОВЕРЯТЬ **ТОЛЬКО** тот API сервис, ключ которого был добавлен или изменен в файле `.env.local` в текущий момент. Не проверять все остальные (старые) ключи каждый раз! Использовать `scripts/check-ai-cascade.mjs` для быстрой валидации.
- **AI-Каскад (Надежность)**: При внедрении новых функций ИИ использовать единый оркестратор `src/lib/ai/gemini.ts`. Он обеспечивает автоматический переход (fallback) между провайдерами: Gemini -> OpenRouter -> Anthropic -> Perplexity -> Moonshot -> OpenAI.
