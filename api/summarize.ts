    ...
    const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`,
        {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [{
                        text: `Ты — ИИ-аналитик и редактор. Прочитай данный текст (это может быть описание видео на YouTube или пост в Telegram) и напиши краткое саммари (1-2 предложения) на РУССКОМ языке. 
Очень важно: 
1. ИГНОРИРУЙ любые технические ссылки (t.me, http, me/...), промокоды, призывы подписаться на канал, таймкоды и прочий мусор. Возвращай ТОЛЬКО СУТЬ самого полезного контента. Без ссылок, без url, без эмодзи.
2. В поле "mentions" ОБЯЗАТЕЛЬНО извлеки ТОЛЬКО названия нейросетей, языковых моделей (LLM) и специфических AI-сервисов (например: ChatGPT, Claude, Midjourney, v0, Bolt.new, Perplexity, OpenAI, Anthropic и т.д.). СТРОГО ЗАПРЕЩЕНО писать обычные инструменты разработчиков, языки программирования и веб-фреймворки (НЕ ПИШИ React, Python, Go, Java, Next.js, Vite, Tailwind, Node.js, Express, HTML, CSS и т.д.). Нас интересует ТОЛЬКО AI!


Верни ТОЛЬКО валидный JSON без markdown и блоков кода (никаких маркдаун-кавычек json!):
{
  "summary": "Глубокое и развернутое саммари (4-6 предложений) на русском языке. Максимально подробно опиши суть видео/поста: о чем он, какие именно идеи, инструменты или подходы в нем обсуждаются, проблемы, которые он решает.",
  "tags": ["тег1", "тег2"],
