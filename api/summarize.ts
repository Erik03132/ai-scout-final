    ...
    const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`,
        {
            ...
            body: JSON.stringify({
                contents: [{
                    parts: [{
                        text: `Ты — редактор новостей. Прочитай данный текст (это может быть описание видео на YouTube или пост в Telegram) и напиши краткое саммари (1-2 предложения) на РУССКОМ языке. 
Очень важно: ИГНОРИРУЙ любые технические ссылки (t.me, http, me/...), промокоды, призывы подписаться на канал, таймкоды и прочий мусор. Возвращай ТОЛЬКО СУТЬ самого полезного контента.
Без ссылок, без url, без эмодзи.

Верни ТОЛЬКО валидный JSON без markdown и блоков кода(без \`\`\`json):
{
