...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    1. ИСПОЛЬЗУЙ ПОИСК + ВНУТРЕННИЕ ЗНАНИЯ Модели. Если инструмент очень новый и данных в поиске мало, составь описание на основе его названия и контекста (как эксперт-аналитик). Фраза "Нуждается в уточнении" — ТОЛЬКО если вообще нет зацепок.
    2. КОНКРЕТИКА: 
       - Lyria (Google DeepMind) — это музыка и аудио.
       - Lovable — это билдер веб-приложений (React/Supabase).
       - Gemini Canvas — это интерактивная среда для работы с кодом и контентом внутри Gemini.
    3. ОТВЕЧАЙ ТОЛЬКО НА РУССКОМ.
    4. ВЕРНИ ТОЛЬКО ЧИСТЫЙ JSON.

    СТРУКТУРА JSON:
    - description: Минимум 3-4 абзаца. Подробно: что это, функционал, для кого.
