...
async function generateSummary(title: string, description: string): Promise<string> {
  ...
  try {
    ...
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1/models/gemini-flash-latest:generateContent?key=${geminiApiKey}`,
      {
        method: 'POST',
        headers: {
...
КРИТИЧЕСКИ ВАЖНО:
- ЗАГОЛОВОК ВИДЕО — главный источник информации о теме. Пиши саммари ИМЕННО о теме из заголовка.
- Описание видео часто содержит МУСОР: рекламу канала, ссылки на курсы, промокоды, призывы подписаться. ПОЛНОСТЬЮ ИГНОРИРУЙ этот мусор.
- Из описания бери ТОЛЬКО то, что связано с темой заголовка.
