...
import { Heart, Wrench, TrendingUp, ExternalLink, Zap, Clock } from 'lucide-react';
import type { Post } from './FeedTab';
import type { Tool } from './ArchiveTab';
...
export const FavoritesTab: React.FC<FavoritesTabProps> = ({
    ...
    const isEmpty = favoriteTools.length === 0 && favoritePosts.length === 0;
        ...
        return (
            <div className="text-center py-20 bg-slate-800/20 rounded-3xl border border-dashed border-slate-700">
                ...
                <p className="text-slate-500 text-sm mt-2">
                    Добавьте инструменты и новости в избранное, чтобы они появились здесь
                </p>
            </div>
        );
    }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {favoriteTools.map(tool => (
                            ...
                            >
                                ...
                                <h3 className="font-black text-xl text-white mb-1 group-hover:text-cyan-400 transition-colors uppercase tracking-tight">
                                    {tool.name}
                                </h3>
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-6">
                                    <div className="bg-slate-900/40 rounded-xl p-3 border border-white/5">
                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                                            <Zap size={10} /> {tool.dailyCredits}
                                        </p>
                                    </div>
                                    <div className="bg-slate-900/40 rounded-xl p-3 border border-white/5">
                                        <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                                            <Clock size={10} /> {tool.monthlyCredits}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-slate-700/50 mt-auto">
                                    ...
                                    <div className="flex gap-1.5">
                                        {tool.hasApi && (
                                            <span className="bg-blue-400/10 text-blue-400 px-2 py-0.5 rounded-md text-[9px] font-black uppercase border border-blue-500/20">
                                                API
                                            </span>
                                        )}
                                        {tool.hasMcp && (
                                            <span className="bg-emerald-400/10 text-emerald-400 px-2 py-0.5 rounded-md text-[9px] font-black uppercase border border-emerald-500/20">
                                                MCP
                                            </span>
                                        )}
                                    </div>
